...
async function checkAvailability({ date }) {
  const rows = await db.all(
    "SELECT time FROM appointments WHERE date = ? AND status = 'activo'",
    [date]
  );
...
async function createAppointment({ date, time, patient_name, patient_phone, service }) {
  const conflict = await db.get(
    "SELECT id FROM appointments WHERE date = ? AND time = ? AND status = 'activo'",
    [date, time]
  );
  if (conflict) {
  ...
  const result = await db.run(
    'INSERT INTO appointments (date, time, patient_name, patient_phone, service, status) VALUES (?, ?, ?, ?, ?, ?)',
    [date, time, patient_name, patient_phone || null, service, 'activo']
  );
  return { success: true, appointment: { id: result.lastInsertRowid, date, time, patient_name, patient_phone, service, status: 'activo' } };
}

async function cancelAppointment({ appointment_id }) {
  const appt = await db.get('SELECT * FROM appointments WHERE id = ?', [appointment_id]);
  ...
  await db.run("UPDATE appointments SET status = 'cancelado' WHERE id = ?", [appointment_id]);
...
async function getPatientAppointments({ patient_name, patient_phone }) {
  if (patient_phone) {
    const rows = await db.all('SELECT * FROM appointments WHERE patient_phone LIKE ? ORDER BY date, time', [`%${patient_phone}%`]);
    return { appointments: rows };
  }
  if (patient_name) {
    const rows = await db.all('SELECT * FROM appointments WHERE patient_name LIKE ? ORDER BY date, time', [`%${patient_name}%`]);
    return { appointments: rows };
  }
...
async function chat(conversationId, userContent) {
  const history = await db.all(
    'SELECT role, content, tool_call_id, tool_calls, name FROM messages WHERE conversation_id = ? ORDER BY id',
    [conversationId]
  );
  ...
  while (iterations < MAX_ITER) {
    ...
    if (choice.finish_reason === 'tool_calls' && assistantMsg.tool_calls?.length) {
      ...
      for (const toolCall of assistantMsg.tool_calls) {
        ...
        await db.run(
          'INSERT INTO messages (conversation_id, role, content, tool_call_id, name) VALUES (?, ?, ?, ?, ?)',
